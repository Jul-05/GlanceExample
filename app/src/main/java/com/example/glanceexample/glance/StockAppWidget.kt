...
import androidx.glance.text.FontWeight.Companion.Medium
...
class StockAppWidget : GlanceAppWidget() {
    ...
    private fun StockDisplay(stateCount: Float) {
        ...
        Text(PriceDataRepo.ticker, style = TextStyle(fontSize = 24.sp, fontWeight = FontWeight.Bold))
        Text(text = String.format(Locale.getDefault(), "%.2f", stateCount), style = textStyle)
        Text(text = "${PriceDataRepo.change}%", style = textStyle)
    }
